...
def update_all_messages():
    ...
    msg += f"<b>ซีพียู:</b> <code>{psutil.cpu_percent()}%</code>" \
           f" <b>แรม:</b> <code>{psutil.virtual_memory().percent}%</code>" \
           f" <b>ดิสก์:</b> <code>{psutil.disk_usage('/').percent}%</code>"
    with download_dict_lock:
        ...
        msg += f"\n<b>เหลือ:</b> <code>{free}</code> | <b>เวลาทำงาน:</b> <code>{currentTime}</code>\n<b>DL:</b> <code>{dlspeed}/s</code> 🔻 | <b>UL:</b> <code>{ulspeed}/s</code> 🔺\n"
    with status_reply_dict_lock:
...
def sendStatusMessage(msg, bot):
    ...
    progress += f"<b>ซีพียู:</b> <code>{psutil.cpu_percent()}%</code>" \
           f" <b>แรม:</b> <code>{psutil.virtual_memory().percent}%</code>" \
           f" <b>ดิสก์:</b> <code>{psutil.disk_usage('/').percent}%</code>"
    with download_dict_lock:
        dlspeed_bytes = 0
        ...
        dlspeed = get_readable_file_size(dlspeed_bytes)
        ulspeed = get_readable_file_size(uldl_bytes)
        progress += f"\n<b>เหลือ:</b> <code>{free}</code> | <b>UPTIME:</b> <code>{currentTime}</code>\n<b>DL:</b> <code>{dlspeed}/s</code> 🔻 | <b>UL:</b> <code>{ulspeed}/s</code> 🔺\n"
    with status_reply_dict_lock:
