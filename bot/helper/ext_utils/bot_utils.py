...
def get_readable_message():
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[start:], start=1):
            msg += f"<b>ชื่อไฟล์:</b> <code>{download.name()}</code>"
            msg += f"\n<b>สถานะ:</b> <i>{download.status()}</i>"
            if download.status() not in [
                ...
                if download.status() == MirrorStatus.STATUS_CLONING:
                    ...
                    msg += f"\n<b>ดาวน์โหลด:</b> <code>{get_readable_file_size(download.processed_bytes())}</code> of <code>{download.size()}</code>"
                msg += f"\n<b>ความเร็ว:</b> <code>{download.speed()}</code> <b>เวลาที่เหลือ:</b> <code>{download.eta()}</code>"
                try:
                    msg += f"\n<b>กำลังส่ง:</b> <code>{download.aria_download().num_seeders}</code>" \
                           f" | <b>เพียร์:</b> <code>{download.aria_download().connections}</code>"
                except:
                    pass
                try:
                    msg += f"\n<b>กำลังส่ง:</b> <code>{download.torrent_info().num_seeds}</code>" \
                           f" | <b>กำลังรับ:</b> <code>{download.torrent_info().num_leechs}</code>"
                except:
                    pass
                msg += f"\n<b>คำสั่งยกเลิก:</b> <code>/{BotCommands.CancelMirror} {download.gid()}</code>"
            msg += "\n\n"
